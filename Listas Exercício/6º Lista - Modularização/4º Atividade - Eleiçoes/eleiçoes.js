...
function votação(vetor) {
  ...
  let posicao = politicos.findIndex(
    (item) => item.codigo == votacao.partido && item.nome == votacao.candidato
  );
  if (posicao != -1) {
...
function maisVotado(vetor) {
  ...
  console.log(
    `O candidato mais votado é ${maisVotado.nome} com ${maisVotado.votos} votos`
  );
}
