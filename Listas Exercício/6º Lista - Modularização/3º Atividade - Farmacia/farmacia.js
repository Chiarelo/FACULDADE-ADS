...
let cadastroRemedio = (farmacias, remedios) => {
  ...
  while (!farmacias.some((item) => item.codigo === objeto.codFarmacia)) {
    objeto.codFarmacia = parseInt(
      prompt("Farmácia não existe. Informe novamente o código")
    );
  }
  let indice = remedios.findIndex(
    (item) =>
      item.codFarmacia === objeto.codFarmacia && item.nome === objeto.nome
  );
  if (indice === -1) {
    // remédio não existe
    remedios.push(objeto); // insere remédio
  } else {
    // já existe, atualiza estoque
...
let vendaRemedio = (remedios) => {
  ...
  if (
    remedios.some(
      (item) =>
        item.codFarmacia === venda.farmacia && item.nome === venda.remedio
    )
  ) {
...
let relatorio = (remedios) => {
  ...
  for (let i = 0; i < remedios.length; i++) {
    if (remedios[i].codFarmacia === codigo) {
      relatorio =
        relatorio +
        `Remédio: ${remedios[i].nome} Estoque: ${remedios[i].qtde}\n`;
    }
  }
  alert(relatorio);
};
